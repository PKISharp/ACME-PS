function New-CertificateKey {
    ...
    if($PSCmdlet.ParameterSetName -eq "ECDsa") {
        $certificateKey = [ACMESharpCore.Crypto.ICertificateKey]([ACMESharpCore.Crypto.ECDsaAdapter]::new($ECDsaHashSize));
        Write-Verbose "Created new ECDsa certificate key with hash size $ECDsaHashSize";
    } else {
        $certificateKey = [ACMESharpCore.Crypto.ICertificateKey]([ACMESharpCore.Crypto.RSAAdapter]::new($RSAHashSize, $RSAKeySize));
